...
This repo contains the MaticonOffice Docs React component which integrates [MATICONOFFICE Document Server](https://github.com/MaticonOffice/DocumentServer) into [React](https://react.dev/) projects.

**Please note**: Before working with this component, you need to install MaticonOffice Docs. To do so, you can use [Docker](https://github.com/MaticonOffice/Docker-DocumentServer) (recommended).

## Prerequisites

This procedure requires [Node.js (and npm)](https://nodejs.org/en).

## Creating the demo React application with MATICONOFFICE Docs editor

This procedure creates a [basic React application](https://github.com/facebook/create-react-app) and installs an MATICONOFFICE Docs editor in it.
...
3. Install MATICONOFFICE Docs React component from **npm** and save it to the *package.json* file with *--save*:
```
...
This JavaScript file will create the *App* component containing the MATICONOFFICE Docs editor configured with basic features.

5. Test the application using the Node.js development server:
* To start the development server, navigate to the *maticonoffice-react-demo* directory and run:
```
npm run start
```
...
1. Navigate to the *maticonoffice-react-demo* directory and run:
```
npm run build
```
The *build* directory will be created with a production build of your app.

2. Copy the contents of the *maticonoffice-react-demo/build* directory to the root directory of the web server (to the *maticonoffice-react-demo* folder).
...
| `documentServerUrl` | string | null | yes | Address of MATICONOFFICE Document Server. |
| `shardkey` | string \| boolean | true | no | The string or boolean parameter required to request load balancing during collaborative editing: all users editing the same document are served by the same server. [Shard key](https://api.maticonoffice.ru/docs/docs-api/get-started/how-it-works/#shard-key)|
| `config` | object | null | yes | Generic configuration object for opening a file with token. [Config API](https://api.maticonoffice.ru/docs/docs-api/usage-api/config/) |
...
In case you have any issues, questions, or suggestions for the MaticonOffice Document Server React component, please refer to the [Issues](https://github.com/MaticonOffice/document-editor-react/issues) section.
